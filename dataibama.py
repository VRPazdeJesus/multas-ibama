...
def main():
    ...
    if response.status_code == 200:
        ...
        for category in categories:
            ...
            row = {'municipio': list_municipio, 'nomeRazaoSocial': list_nomeRazaoSocial, 'valorAuto': list_valorAuto, 
                'dataAuto': list_dataAuto, 'situcaoDebito': list_situcaoDebito}
            df = pd.DataFrame(row,columns=['municipio', 'nomeRazaoSocial', 'valorAuto', 'dataAuto', 'situcaoDebito'])
            df.to_csv('%s.csv' % category)
            print("Multas relaciondados a %s foram salvas na tabela!" % category)
